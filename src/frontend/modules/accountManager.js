class AccountManager {
  constructor() {
    ...
    window.glorpClient.settings.toggleAccountManager = (enabled) =>
      this.toggle(enabled);
  ...
  createNewAccount() {
    ...
    if (
      this.accounts.some(
        (account) => this.decode(account.username) === username
      )
    )
      return;
  ...
  handleAccountSelection(element) {
    const account = this.accounts.find(
      (acc) => this.decode(acc.username) === element.textContent
    );
    ...
    setTimeout(() => {
      ...
      document
        .querySelector(".io-button.io-button--accept.svelte-13ld0w6")
        .click();
      this.checkCaptcha();
    }, 1);
  }
  ...
  resetForm() {
    document.querySelector("#color-picker").value = `#${Math.floor(
  ...
  removeAccount(event) {
    ...
    if (clickedElement.classList.contains("accountHolder")) {
      const index = this.accounts.findIndex(
        (account) =>
          this.decode(account.username) === clickedElement.textContent
      );
      if (index > -1) {
