...
window.glorpClient.settings.changeSetting = (id, value, fromSlider) => {
  ...
  switch (id) {
    ...
    case "menuTimer":
      ...
      break;
      case "rich_presence": {
        window.chrome.webview.postMessage(`rpc-update,${value}`);
        break;
      }                    
  }
...
class SettingsManager {
  ...
  init() {
    ...
    this.settingsWindow.getSettings = (...args) =>
      origGetSettings
        .call(this.settingsWindow, ...args)
        .replace(/^<\/div>/, "") + this.getCSettings();
  ...
  getCSettings() {
    ...
    Object.keys(cSettings).forEach((entry) => {
      ...
      if (this.settingsWindow.settingSearch && !this.searchMatches(setting))
        return;
