...
use windows::{
    Win32::{Foundation::*, UI::Shell::ShellExecuteW, UI::WindowsAndMessaging::*},
    core::*,
};
...
fn obs_plugin_paths() -> Vec<PathBuf> {
...
fn set_obs_plugin_installed(webview: &ICoreWebView2, install: bool) {
	let result = (|| -> std::io::Result<String> {
		let paths = obs_plugin_paths();
		let Some(destination) = paths.first() else {
			return Err(std::io::Error::new(std::io::ErrorKind::NotFound, "OBS was not found in Program Files"));
		};
		if install {
			...
			Ok(format!("OBS plugin installed to {}", destination.display()))
		} else if destination.exists() {
			fs::remove_file(destination)?;
			Ok("OBS plugin removed".to_string())
		} else {
			for alternate in paths.iter().skip(1) {
				if alternate.exists() {
					fs::remove_file(alternate)?;
					return Ok("OBS plugin removed".to_string());
				}
			}
	...
	let (ok, message) = match result {
		...
		Err(error) if error.kind() == std::io::ErrorKind::PermissionDenied => {
			...
			let launched = exe.is_ok_and(|exe| unsafe {
			...
			if launched {
				...
				(false, format!("OBS plugin operation failed: {error}"))
			}
		}
		Err(error) => (false, format!("OBS plugin operation failed: {error}")),
	};
	if !ok {
	...
	if let Ok(json) = serde_json::to_string(&payload) {
		unsafe { webview.PostWebMessageAsJson(PCWSTR(utils::create_utf_string(json).as_ptr())).ok(); }
	}
}

pub fn run_obs_plugin_operation(install: bool) -> std::io::Result<()> {
	let paths = obs_plugin_paths();
	let Some(destination) = paths.first() else {
		return Err(std::io::Error::new(std::io::ErrorKind::NotFound, "OBS was not found in Program Files"));
	};
...
pub fn handle_web_message(
	...
	crate::debug_print(format!("web message: {message_string}"));
    ...
    match parts.as_slice() {
		...
		["obs-plugin", value] => {
			...
			set_obs_plugin_installed(webview, install);
			if !install {
				crate::CONFIG.lock().unwrap().set("obsCapturePlugin", false);
			}
		}
